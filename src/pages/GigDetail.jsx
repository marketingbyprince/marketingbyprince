...
export default function GigDetail() {
  ...
  const platformCount = Math.max(1, selPlatformIds.size)
  ...
  const handleSubmit = async e => {
    ...
    const { error } = await supabase.from('leads').insert([{
      ...
      source_page:             `/gigs/${id}`,
      selected_gig_id:         gig.id,
      selected_gig_package_id: selPkgId || null,
      selected_addons:         selAddonObjs.length
        ? selAddonObjs.map(a => ({ id: a.id, name: a.name, price: a.price }))
        : null,
      selected_platforms:      selPlatformObjs.length
        ? selPlatformObjs.map(p => ({ id: p.id, name: p.name }))
        : null,
      estimated_price: grandTotal > 0 ? grandTotal : null,
      is_read:         false,
    }])
  ...
  if (!gig) return (
    <div className="min-h-screen flex items-center justify-center bg-soft">
      <div className="text-center">
        ...
        <Link to="/gigs" className="btn-primary btn-md">← Back to Gigs</Link>
      </div>
    </div>
  )

  const stepNum = n => n  // steps shift if platforms are absent
      ...
      <div className="section-narrow">
              ...
              {packages.map(pkg => {
                ...
                return (
                  <div
                    ...
                    className={`relative flex flex-col rounded-2xl p-6 border-2 cursor-pointer select-none transition-all duration-150 ${
                      isSelected
                  ...
                  >
                    {isFeatured && !isSelected && (
                      <span
                        className="absolute -top-3.5 left-1/2 -translate-x-1/2 text-xs font-extrabold text-white px-3 py-0.5 rounded-full whitespace-nowrap"
                    ...
                    {/* Tier label */}
                    <span className={`text-xs font-extrabold uppercase tracking-wider mb-2 ${
                    ...
                    {/* Package name */}
                    <h3 className="font-extrabold text-deep text-base leading-snug mb-4">
                      {pkg.name || pkg.tier}
                    </h3>

                    {/* Price */}
                    <div className="mb-5">
                      <div className="text-price-val"
                      ...
                      {pkg.delivery_days && (
                        <p className="text-body-sm text-gray-400 font-medium mt-1">
                          ⏱ {pkg.delivery_days}-day setup
                        </p>
                      )}
                    </div>
                      ...
                      <ul className="flex-1 space-y-2 mb-6">
                        {pkg.features.map((f, i) => (
                          <li key={i} className="flex items-start gap-2 text-body-sm text-gray-600">
                            <span className="shrink-0 mt-0.5 font-bold" style={{ color: 'var(--accent)' }}>
                              ✓
                            </span>
                            {f}
                          </li>
                    ...
                    {/* Select button */}
                    <div
                      className={`mt-auto text-center py-2.5 rounded-xl font-bold text-sm border-2 transition-all ${
                        isSelected || isFeatured
                          ? 'text-white border-transparent'
                          : 'border-gray-200 text-deep hover:border-accent hover:text-accent'
                      }`}
        ...
        {platforms.length > 0 && (
          <section className="mb-14">
            <StepHeading
              step="2"
              title="Select Advertising Platforms"
              sub="Your package price multiplies by the number of platforms you choose"
            />
              ...
              <div className="card p-4 border-l-4" style={{ borderLeftColor: 'var(--accent)' }}>
                <p className="text-body-sm text-gray-600">
                  <span className="font-extrabold text-deep">
                    {selPlatformIds.size} platform{selPlatformIds.size > 1 ? 's' : ''}
                  </span>
                  {' '}×{' '}
                  ...
                  <span className="font-extrabold text-lg" style={{ color: 'var(--accent)' }}>
                    ${basePrice.toLocaleString()}/mo base
                  </span>
                </p>
                <p className="text-xs text-gray-400 mt-1 font-medium">
                  Platforms: {selPlatformObjs.map(p => p.name).join(', ')}
                </p>
              </div>
            )}
          </section>
        ...
        {addons.length > 0 && (
          <section className="mb-14">
            <StepHeading
              step={platforms.length > 0 ? '3' : '2'}
              title="Add-Ons"
              sub="Optional extras — click to include in your inquiry"
            />
              ...
              {addons.map(addon => {
                ...
                return (
                  ...
                  >
                    ...
                    {addon.description && (
                      <p className="text-body-sm text-gray-500 leading-relaxed mt-1.5">
                        {addon.description}
                      </p>
                    )}
                  </button>
            ...
            {hasSelection && (
              <div className="mb-8 pb-6 border-b border-gray-100">
                ...
                <div className="space-y-2.5">
                  {selPkgObj && (
                    <SummaryRow
                      label={`${selPkgObj.name || selPkgObj.tier} × ${selPlatformIds.size || 1} platform${(selPlatformIds.size || 1) > 1 ? 's' : ''}`}
                      value={`$${basePrice.toLocaleString()}/mo`}
                    />
                  )}
                  {selAddonObjs.map(a => (
                    <SummaryRow
                      key={a.id}
                      label={a.name}
                      value={a.price > 0 ? `+$${Number(a.price).toLocaleString()}` : 'Included'}
                    />
                  ))}
            ...
            {status === 'success' ? (
              <div className="text-center py-8">
                ...
                <p className="text-body text-gray-500">
                  I&rsquo;ll get back to you within 24 hours with a detailed proposal.
                </p>
              </div>
            ) : (
              <>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  {!selPkgObj && (
                    <div
                      ...
                      style={{ backgroundColor: 'var(--warn-canvas)', color: '#92400E', border: '1px solid var(--warn-border)' }}
                    >
                      💡 Select a package above to include it in your inquiry.
                    </div>
                  )}
