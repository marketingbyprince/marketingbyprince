...
export default function ServicesSeoListPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      supabase.from('seo_page_meta').select('seo_score, meta_title, meta_description, og_image, schemas').eq('content_type', 'services').is('content_id', null).single(),
    ]).then(([{ data: servicesData }, { data: seoData }, { data: pageData }]) => {
    ...
    <div className="p-6 md:p-8 max-w-5xl mx-auto space-y-8">
      ...
      <div className="rounded-2xl border p-5 flex items-center justify-between gap-6"
        ...
        <div className="flex items-center gap-5">
          ...
          <div>
            <p className="text-white font-bold">Services Listing Page</p>
            <p className="text-xs text-gray-400 mt-0.5">SEO for the main <code className="text-orange-400">/services</code> page</p>
        ...
        <Link href="/admin/seo/services/page-seo"
      ...
      <div>
        <h2 className="text-white font-bold text-sm uppercase tracking-widest mb-3">Individual Services</h2>
        {loading ? (
          ...
          <div className="space-y-8">
            {Object.entries(grouped).map(([pillar, items]) => (
              <div key={pillar}>
                <h3 className="text-gray-400 font-bold text-xs uppercase tracking-widest mb-2 px-1">{pillar}</h3>
