...
export async function getSeoSchemas({ contentType, contentId = null, extraNodes = [] }) {
  ...
  if (Array.isArray(meta?.aeo_faq_schema) && meta.aeo_faq_schema.length) {
    nodes.push(buildFaqPageSchema(meta.aeo_faq_schema, {
...
// Person and BreadcrumbList nodes already in the graph.
export function buildProfilePageSchema({ id, url, name, personId, breadcrumbId }) {
  return {
    '@id': id,
    '@type': 'ProfilePage',
    url,
